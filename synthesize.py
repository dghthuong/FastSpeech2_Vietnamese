...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--ref_voice",
        type=str,
        default="./audio_samples/sontung.wav",
    )
    parser.add_argument(
        "--ref_lib_config",
        type=str,
        default="/media/cxnam/cxnam_folder/SpeakerEmbedding/config/speaker_encoder.yaml",
    )
    ...
    # Ref voice
    config = yaml.load(open(args.ref_lib_config, "r"), Loader=yaml.FullLoader)

    model_wespeaker = wespeaker.load_model(config["path"]["model_folder_path"])
    model_wespeaker.set_gpu(0)

    embedding = model_wespeaker.extract_embedding(args.ref_voice)
    ...
    if args.mode == "single":
        ...
        batchs = [(ids, raw_texts, speakers, texts, text_lens, max(text_lens), np_embedding)]
